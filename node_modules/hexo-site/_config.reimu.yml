...
injector:
  ...
  head_end: |-
    <style>
      ...
      .post-info{display:flex;flex-direction:column;align-items:center}
      .post-title{order:1;text-align:center;font-size:2rem;line-height:1.3;margin:0.6rem 0}
      .post-brief{order:2;text-align:center;margin:0.4rem 0 0.6rem}
      .post-meta{order:3;display:flex;gap:8px;flex-wrap:wrap;justify-content:center;text-align:center}
      .card{max-width:360px;margin:16px auto;border-radius:16px;overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,.08);background:#111;color:#fff}
      .card .top-section{position:relative;padding:16px;background:linear-gradient(135deg,#2b2b2b,#1a1a1a)}
      .card .top-section .border{position:absolute;inset:0;border:2px solid rgba(255,255,255,.08);border-radius:16px;pointer-events:none}
      .card .icons{display:flex;justify-content:space-between;align-items:center}
      .card .logo .svg{width:48px;height:48px}
      .card .social-media .svg{width:22px;height:22px;margin-left:10px;fill:#fff;opacity:.85}
      .card .bottom-section{padding:16px}
      .card .title{display:block;font-weight:700;letter-spacing:.06em;margin-bottom:10px}
      .card .row{display:flex;gap:12px}
      .card .item{flex:1;display:flex;flex-direction:column;align-items:flex-start}
      .card .big-text{font-size:20px;font-weight:700}
      .card .regular-text{font-size:12px;opacity:.8}
    </style>
...
footer:
  since: 2021 # 2020-current year
  powered: false
  count: false # show word count and reading time
  busuanzi: true
  icon:
...
social:
  email: mailto:torch@318670.xyz
  github: https://github.com/qingkong143
  ...
  bilibili: https://space.bilibili.com/1983066952
...
home_categories:
  enable: false
  content:
    - categories: 原创诗 # string (single-layer category) or array (multi-layer category)
      cover: https://pic.netbian.com/uploads/allimg/250804/201934-1754309974780c.jpg # empty means random cover
    - categories: Hexo
      cover: https://pic.netbian.com/uploads/allimg/250804/202713-1754310433b0f4.jpg
...
player:
  ...
  position: after_sidebar # before_sidebar / after_sidebar / after_widget
