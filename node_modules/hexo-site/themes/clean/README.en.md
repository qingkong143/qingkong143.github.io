...
#### Bangumi Page Configuration Notes

The `bangumi` section in `_config.yml` only controls page-level copy (title/subtitle/description).

Maintain bangumi entries in the bangumi page front-matter `bangumi` field; optional experimental Bilibili auto-fetch should be configured in the same page front-matter `bilibili` field.
