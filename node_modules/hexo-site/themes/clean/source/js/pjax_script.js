...
if (window.__cleanSidebarTopScrollHandler) {
  window.off("scroll", window.__cleanSidebarTopScrollHandler);
}

window.__cleanSidebarTopScrollHandler = () => {
...
window.on("scroll", window.__cleanSidebarTopScrollHandler);
...
function tocInit() {
  ...
  const activateNavByIndex = (index) => {
    ...
    if (!_$(".sidebar-toc-sidebar").classList.contains("hidden")) {
      ...
      const top = tocWrapper.scrollTop + target.offsetTop - tocWrapper.offsetHeight / 2;
      tocWrapper.scrollTo({
        top: Math.max(0, top),
      });
    }
  ...
  if (window.__cleanTocObserver) {
    window.__cleanTocObserver.disconnect();
  }
  ...
  window.__cleanTocObserver = observer;
