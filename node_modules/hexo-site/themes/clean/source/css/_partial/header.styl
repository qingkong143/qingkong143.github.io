#header {
  ...
  &:before {
    z-index: -1;
    background-image: var(--header-shared-visual);
...
#header-nav {
  ...
  background: transparent;
  box-shadow: none;
  transition: background 1s, top 0.3s, box-shadow 0.3s;
  z-index: 200;
...
.dark-mode-switch-input:checked ~ .dark-mode-icon-sun {
  opacity: 0;
}

.dark-mode-switch-input:checked ~ .dark-mode-icon-moon {
  opacity: 1;
}

.dark-mode-switch-input:focus-visible ~ .dark-mode-icon-sun, .dark-mode-switch-input:focus-visible ~ .dark-mode-icon-moon {
  filter: drop-shadow(0 0 6px var(--red-2));
}
...
[data-theme='dark'] {
  .main-nav-link, .nav-icon, .main-nav-link-wrap, .dark-mode-switch {
    color: var(--red-2);
  }
}
