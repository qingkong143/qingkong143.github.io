...
<%_ } else { _%>
  ...
  <body>
    <%_
      const bannerSrcsetEnabled = theme.banner_srcset && theme.banner_srcset.enable;
      const fallbackBanner = theme.banner ? url_for(theme.banner, {relative: false}) : '';
      const fallbackSrcsetBanner = bannerSrcsetEnabled && theme.banner ? url_for(theme.banner, {relative: false}) : fallbackBanner;
      let sharedHeroBackground = '';
      if (page.cover && typeof page.cover === 'string' && !page.cover.startsWith('rgb')) {
        sharedHeroBackground = url_for(page.cover, {relative: false});
      } else if (is_post() && theme.cover && typeof theme.cover === 'string' && theme.cover !== false && !theme.cover.startsWith('rgb')) {
        sharedHeroBackground = url_for(theme.cover, {relative: false});
      } else if (fallbackSrcsetBanner) {
        sharedHeroBackground = fallbackSrcsetBanner;
      } else if (fallbackBanner) {
        sharedHeroBackground = fallbackBanner;
      }
    _%>
    ...
    <div id="container">
      <%_ if (theme.global_background.enable && sharedHeroBackground) { _%>
        <div class="global-hero-background" style="background-image:url('<%- sharedHeroBackground %>')"></div>
      <%_ } _%>
      <div id="wrap">
        <%- partial('_partial/header', {post: page, sharedHeroBackground}) %>
        <div id="content" aria-label="Page content" class="<%= is_home() ? 'content-home' : 'content-page' %> <%= (page.sidebar === 'left' || page.sidebar === undefined && theme.sidebar === 'left') ? 'sidebar-left' : '' %> <%= (page.sidebar === 'right' || page.sidebar === undefined && theme.sidebar === 'right') ? 'sidebar-right' : '' %>">
