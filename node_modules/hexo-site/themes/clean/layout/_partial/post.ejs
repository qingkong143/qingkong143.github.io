<div class="post-wrapper">
  <div class=<% if (even === true) { %>"post-wrap left"<% } else { %>"post-wrap right"<% } %> data-aos="<%= theme.animation.options.home.post %>">
    ...
    <%_
      const postcardOriginalEnabled = theme.postcard === 'enable' || theme.postcard?.enable === true || theme.postcard?.excerpt === true;
      const manualSummary = post.summary || post.description || post.brief;
      const summaryOnly = manualSummary && !postcardOriginalEnabled;
    _%>
    <div class="post-info<%= summaryOnly ? ' summary-only' : '' %>">
      <div class="post-meta">
      ...
      <%_ if (manualSummary) { _%>
        <div class="post-brief"><%= stripHtml(manualSummary) %></div>
      <%_ } _%>
      <%_ if (postcardOriginalEnabled) { _%>
