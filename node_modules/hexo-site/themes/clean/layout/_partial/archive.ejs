...
<%_ } else { _%>
<%_
  const archivePageTitle = is_archive()
    ? __('archives')
    : is_category()
      ? __('categories')
      : is_tag()
  ...
  let archivePageSubtitle = null;
  if (is_archive()) {
    if (is_month()) {
      archivePageSubtitle = page.year + '/' + page.month;
    } else if (is_year()) {
      archivePageSubtitle = page.year;
    }
  } else if (is_category()) {
    archivePageSubtitle = page.category;
  } else if (is_tag()) {
    archivePageSubtitle = page.tag;
  }
_%>
<div class="archives-outer-wrap" data-aos="<%= theme.animation.options.archive.whole %>">
  <header class="archive-page-header">
    <h1 class="archive-page-title"><%= archivePageTitle %></h1>
    <%_ if (archivePageSubtitle) { _%>
      <p class="archive-page-subtitle"><%= archivePageSubtitle %></p>
    <%_ } _%>
  </header>
  <%_ if (theme.only_show_capsule_in_index) { _%>
    <%_ if (page.current === 1) { _%>
