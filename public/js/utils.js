const anzhiyu = {
  ...
  catalogActive: function () {
    ...
    if ($list) {
      ...
      catalogListItems.forEach(item => {
        const id = decodeURIComponent(item.id || "").replace(/\/+$/, "") || "/";
        const match = id === "/" ? pathname === "/" : pathname === id || pathname.startsWith(`${id}/`);
        if (match && id.length > best) {
          best = id.length;
          $catalog = item;
        }
      });
      if ($catalog) $catalog.classList.add("selected");
  ...
  musicToggle: function (changePaly = true) {
    if (!navMusicEl) navMusicEl = document.getElementById("nav-music");
    ...
    const menuToggle = document.getElementById("menu-music-toggle");
    ...
    if (anzhiyu_musicPlaying) {
      ...
      if (hoverTips) hoverTips.innerHTML = "音乐已暂停";
  ...
  changeSayHelloText: function () {
    ...
    if (!authorInfoSayHiElement || !greetings.length) return;
